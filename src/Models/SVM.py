...
class Model:
    ...
    def evaluate(self):
        ...
        self.clf.fit(data[TRAIN]["X"], data[TRAIN]["Y"])
        ...
        for data_type in [TRAIN, VALID]:
            y_pred = self.clf.predict(data[data_type]["X"])

            cyanprint(f"[{data_type}] Accuracy... ({time.time() - time_start:.4}s)")
            metrics["accuracy"][data_type] = self.clf.score(data[data_type]["X"], data[data_type]["Y"])
            ...
            for average in [None, "weighted"]:
                prfs = sklearn.metrics.precision_recall_fscore_support(data[data_type]["Y"], y_pred, average=average)
            ...
            for average in [None, "weighted", "samples"]:
                rocauc = sklearn.metrics.roc_auc_score(data[data_type]["Y"], y_pred, average=average)
                metrics["rocauc"][data_type][average] = rocauc

            cyanprint(f"[{data_type}] Confusion Matrix... ({time.time() - time_start:.4}s)")
            metrics["confusion_matrix"][data_type] = confusion_matrix(data[data_type]["Y"], y_pred)

        blueprint(f"Finished ({time.time() - time_start:.4}s)")
        return metrics


class SVMBaseNoWeights(Model):
    def __init__(self):
        super().__init__(svm.SVC(gamma="scale", verbose=1))


class SVMBaseAutoWeights:
    def __init__(self):
        super().__init__(svm.SVC(gamma="scale", class_weight="balanced", verbose=1))
